...
mepath=$(dirname $0)
export jkscripthome=$(dirname ${mepath})
export jenkins_workspace=$(dirname ${jkscripthome})
...
ald_generate_diff(){
	...
	# set directory list to prepare from SVN
	if [ "${country}" = "CORE" ];then
		local_directory_list=${jkscripthome}/core/${country}.${module}.list
	else
		local_directory_list=${jkscripthome}/country_specific/${country}.${module}.list
	fi
	...
	if [ ${module} = "TEMPLATES" ]; then
		...
		do
			echo "$(date) - INFO [${country}] [${module}] [ald_generate_diff]: Copying ${svn_code_home}/${country}/${svn_item} to ${module_src_home}/new_src/${module_src_dir}"
