...
mepath=$(dirname $0)
export jenkins_workspace=$(dirname ${mepath})
...
ald_signon(){
	...
	case ${ald_runtime_status} in
		...
		2 )	if [ ${run_mode} = "FORCE" ]; then
			# terminate running session
				...
				echo "$(date) - INFO [${country}] [${module}] [ald_signon]: ${run_mode} mode. Waiting for ${queue_timeout} minutes for the foriegn session to exit..."
				while [[ ( $(get_ald_runtime_session) -ne 0 || ${wait_time} -eq ${queue_timeout} ) ]];
				do
					sleep 60;
					((wait_time=${wait_time}+1));
				done
				if [[ ( $(get_ald_runtime_session) -ne 0 && ${run_mode} = "QUEUED" ) ]]; then
	...
	# NOTE - need to check whether initializing of ALDCS is required
	# initialize ALDCS
	echo "$(date) - INFO [${country}] [${module}] [ald_signon]: Cleaning up ${tmp_dir}..."
	cleanup_tmp
	#echo "$(date) - INFO [${country}] [${module}] [ald_signon]: Initializing ALDCS in keep alive mode..."
	#aldcs --fork --keep-alive --socket-keep-alive --port 55555 --logfile ${tmp_dir}/aldcs.log --log-all --log-replace

	# sign on to aldon
	if [ ${rc} -ne 0 ];then
		# exit with error
		error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_signon]: Unable initialize ALDCS. Please see log for error"
	else
	...
	if [ ${rc} -ne 0 ];then
		...
		echo "$(date) - INFO [${country}] [${module}] [ald_signon]: Creating ald_src for dev env..."
	...
	if [ ${rc} -ne 0 ];then
		# exit with error
		error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_signon]: Unable to create or clenup ${module_src_home}/ald_src . Please see log for error"
	else
		echo "$(date) - INFO [${country}] [${module}] [ald_signon]: Set development environment..."
	...
	if [ ${rc} -ne 0 ];then
		# exit with error
		error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_signon]: Unable to set development environment. Please see log for error"
	else
		# all okay
...
ald_generate_diff(){
	...
	# define local code directory names
	module_src_home="${ald_devenv_root}/${country}/${module}"
	case ${module} in
		"ONLINE"    )   module_src_dir="online";;
		"SCRIPTS"   )   module_src_dir="scripts";;
		"TEMPLATES" )   module_src_dir="templates";;
	esac

	# prepare local SVN directories
	# NOTE - this section is compatible with PLCE single source structure. since compiled code, 
	# online and scripts are treated in same pattern whereas templates module is slightly 
	# different. for simplicity and online and scripts local directory lists are managed include
	# [contry].ONLINE.list and [country].SCRIPTS.list respectively (e.g. sg.ONLINE.list).
	# these files will be in atlas repo along with this script.

	# set local directory list
	local_directory_list=${mepath}/${country}.${module}.list
	# create new_src
	rc=0
	mkdir ${module_src_home}/new_src
	mkdir ${module_src_home}/new_src/${module_src_dir}
	rc=$?
	error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_generate_diff]: Unable to generate diff reports. Error creating ${module_src_home}/new_src"
	# read list file and copy sources from SVN directory ${svn_code_home}/${country}
	# TODO: Need to check whether possible to use move instead copy
	cd ${svn_code_home}/${country}
	rc=$?
	error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_generate_diff]: Unable to generate diff reports. Error changing directory to ${svn_code_home}/${country}"
	if [ ${module} = "TEMPLATES" ]; then
		# preparing for templates
		# converting ${country} to lowercase
		country_lc=$(echo ${country} | tr '[:upper:]' '[:lower:]')
		cd "plce-${country_lc}-templates"
		rc=$?
		error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_generate_diff]: Unable to generate diff reports. Error changing directory to plce-${country_lc}-templates in SVN source"
		cp -pR * ${module_src_home}/new_src/${module_src_dir}/
		rc=$?
		error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_generate_diff]: Unable to generate diff reports. Error copying plce-${country_lc}-templates to new_src"
	else
		# preparing for online/scripts
		for svn_item in $(cat ${local_directory_list})
		do
			echo "Copying ${svn_code_home}/${country}/${svn_item} to ${module_src_home}/new_src/${module_src_dir}"
			cp -pR ${svn_item} ${module_src_home}/new_src/${module_src_dir}/
			rc=$?
			error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_generate_diff]: Unable to generate diff reports. Error copying ${svn_code_home}/${country}/${svn_item} to new_src"
		done
	fi
	...
	diff -rqN --strip-trailing-cr new_src/${module_src_dir} ald_src/${module_src_dir} | grep -v .svn | grep -v .cekey | grep -v .aldlme | gawk -F' and ' '{print $1}' | gawk -F'Files ' '{print $2}' > ${country}_${module}_objects_list.txt
	rc=$?
...
ald_checkout(){
	...
	if [ -z "$(cat ${module_src_home}/${country}_${module}_updated_files.txt)" ]; then
		...
		for updated_file in $(cat ${module_src_home}/${country}_${module}_updated_files.txt)
			do
				echo ald checkout -a ${ald_task} -c ${cin_comment} ${updated_file}
				((rc=${rc}+$?))
		done
	...
	# clear any conflicts in checked out objects
	echo "$(date) - INFO [${country}] [${module}] [ald_mark_for_delete]: Starting clearing conflicts for update..."
	for updated_file in $(cat ${module_src_home}/${country}_${module}_updated_files.txt)
	do
		echo ald checkout -a ${asm_task} -c ${cin_comment} ${updated_file}
		((rc=${rc}+$?))
	done
	error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_checkout]: Unable to complete check-out. Error clearing conflict of some of the objects. You may need investigate with LMe"
	return 0
}
...
ald_add_new_objects(){
	...
	if [ -z "$(cat ${module_src_home}/${country}_${module}_new_files.txt)" ]; then
		...
		do
			echo ald add -a ${ald_task} -c ${cin_comment} ${new_file}
			((rc=${rc}+$?))
		done
	fi
...
ald_mark_for_delete(){
	...
	if [ -z "$(cat ${module_src_home}/${country}_${module}_deleted_files.txt)" ]; then
		...
		for deleted_file in $(cat ${module_src_home}/${country}_${module}_deleted_files.txt)
			do
				echo ald checkout -a ${ald_task} -c ${cin_comment} ${deleted_file}
				((rc=${rc}+$?))
		done
	...
	# clear any conflicts in checked out objects
	echo "$(date) - INFO [${country}] [${module}] [ald_mark_for_delete]: Starting clearing conflicts for mark as delete..."
	for deleted_file in $(cat ${module_src_home}/${country}_${module}_deleted_files.txt)
	do
		echo ald checkout -a ${asm_task} -c ${cin_comment} ${deleted_file}
		((rc=${rc}+$?))
	done
	error_abort ${rc} "$(date) - ERROR [${country}] [${module}] [ald_mark_for_delete]: Unable to complete check-out for mark as delete. Error clearing conflict of some of the objects. You may need investigate with LMe" 

	# proceed with mark for delete
	echo "$(date) - INFO [${country}] [${module}] [ald_mark_for_delete]: Starting mark for deletion..."
	for deleted_file in $(cat ${module_src_home}/${country}_${module}_deleted_files.txt)
	do
		echo ald delete ${deleted_file}
		((rc=${rc}+$?))
	done
...
ald_checkin(){
	...
	echo ald checkin -A -c ${cin_comment}
	rc=$?
