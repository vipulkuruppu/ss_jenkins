#!/bin/ksh
################################################################
# PLCE scripts compile/build script for jenkins
...
do
	if [ -n "`grep \"@[A-Za-z0-9\. ]*@\" ${_script}`" ]; then
		...
		if [ ! -d ${ALDON_DIST}/bin/`dirname ${_script}` ]; then
			mkdir -p ${ALDON_DIST}/bin/`dirname ${_script}`
		fi
		cp ${_script}  ${ALDON_DIST}/bin/${_script}
	fi
done
