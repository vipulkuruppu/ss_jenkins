#!/bin/ksh
################################################################
# PLCE cdscripts deploy script for jenkins
...
case ${deploy_env} in
	"sit"	) deploy_path=/prodlib/CLSMY/scripts;
			  deploy_env_label="SIT-BAU";;
	"sit2"	) deploy_path=/prodlib/CLSMY2/scripts;
			  deploy_env_label="SIT2-Project";;
	"sit5"	) deploy_path=/prodlib/CLSMY5/scripts;
...
echo "------------------------------------------------------------------------------------------------" 
echo "$(date) - INFO [${country}] [${module}]: ${CD_SERVER} /TCM housekeep started"
echo "------------------------------------------------------------------------------------------------" 
ssh ${deployid}@${CD_SERVER} << _EOC
/TCM/swdis/TCM_housekeep.sh
_EOC
((retcode=${retcode}+$?))
if [ ${retcode} -ne 0 ];then
	echo "------------------------------------------------------------------------------------------------"
	echo "$(date) - ERROR [${country}] [${module}]: ${CD_SERVER} /TCM housekeep failed"
	echo "------------------------------------------------------------------------------------------------"
	exit ${retcode}
fi
...
cd /TCM/UAT_svr/${release}/deploy/scripts
cp -pR * ${deploy_path}
